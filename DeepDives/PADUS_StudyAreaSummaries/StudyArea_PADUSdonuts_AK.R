...
inputTabAreaGAP <- "S:/Projects/NPCA/Data/Intermediate/GAP_Analysis.gdb/TabArea_GAP_xStudyAreas_AK"
...
inputTabAreaManaged <- "S:/Projects/NPCA/Data/Intermediate/GAP_Analysis.gdb/TabArea_MangNS_xStudyAreas_AK"
...
inputTabAreaGAP$Manager[which(inputTabAreaGAP$Manager=="NA")] <- "Unmanaged"
...
for(i in 1:length(lstStudyAreas)){
  ...
  StudyArea_subset2$Manager <- factor(StudyArea_subset2$Manager, levels = c(lstManagers))
  StudyArea_subset2$Manager <- factor(StudyArea_subset2$Manager, levels =c('NA','UNK','PVT','TRIB','STAT','LOC','FED','DOE','DOD','NGO','BLM','FWS','USFS','NPS'))
  StudyArea_subset2 <- plyr::ddply(StudyArea_subset2, c('Manager')) #sorts the dataframe to match the way the factors are sorted
  StudyArea_subset2$Manager <- fct_rev(StudyArea_subset2$Manager)
  StudyArea_subset2$ymax = cumsum(StudyArea_subset2$PercentArea) #sets top of rectangle for ggplot
  StudyArea_subset2$ymin = c(0, head(StudyArea_subset2$ymax, n=-1)) #sets bottom of rectange for ggplot
  
  StudyArea_subset2 %>%
    ggplot (aes(x=2, ymax=ymax,ymin=ymin, xmax=4, xmin=3, fill = Manager))+
    geom_rect()+
    ggtitle(paste(lstStudyAreas[i],"Study Area")) +
    coord_polar(theta = "y")+ #makes plot circular
    scale_y_reverse()+
