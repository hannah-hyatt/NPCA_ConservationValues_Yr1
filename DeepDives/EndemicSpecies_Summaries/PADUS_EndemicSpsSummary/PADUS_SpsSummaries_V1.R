...
inputTabAreaGAP <- "S:/Projects/NPCA/Data/Intermediate/GAP_Analysis_Species.gdb/TabArea_SpsInsideSAs" # UPDATE Input Tabulate Area table - Managed Lands or GAP status focused
...
inputTabAreaGAP$StudyArea <- gsub("\\(([^()]+)\\)", "\\1",str_extract(inputTabAreaGAP$GAPsts_StudyArea, "\\(([^()]+)\\)"))
...
inputTabAreaGAP$GAPstatus <- sub("\\(.*", "", inputTabAreaGAP$GAPsts_StudyArea)
...
for(i in 1:length(lstStudyAreas)){
  ...
  for(j in 1:length(lstSpecies_subset)){  #
    ...
    StudyAreaSpecies_subset1 <- StudyArea_subset %>%
      group_by( StudyArea, GAPstatus_fin, Scientific_Name,SA_area_m,Rounded_GRank) %>% #NPCA_status_GAP_StudyArea,
    ...
    StudyAreaSpecies_subset3 <- StudyAreaSpecies_subset3[which(StudyAreaSpecies_subset3$PercentArea>10),]
    ...
    StudyAreaSpecies_subset3 %>% #can run from here to...
      ...
      scale_fill_manual(values=c("#27613b", "#869447","#659fb5","#bed5cf","#b1b1b1"), guide = guide_legend(reverse = TRUE)) +
      theme_minimal() +
